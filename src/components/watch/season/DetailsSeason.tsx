...
import { BtnReactionSeason } from "./BtnReactionSeason";
...
export const DetailsSeason = async ({ coverImage, title, seasonNumber, serieId }: Props) => {
  ...
  const currentUserReaction = await getReactionSeasonByUser(serieId, +seasonNumber);
    ...
    <div className="flex flex-col gap-2 details-watch">
      ...
      <div className="flex gap-2">
        {reactions.map(reaction => (
          <BtnReactionSeason key={reaction.label} serieId={serieId} seasonNumber={seasonNumber} typeReaction={reaction.typeReaction} currentUserReaction={currentUserReaction}>
            {reaction.icon}
