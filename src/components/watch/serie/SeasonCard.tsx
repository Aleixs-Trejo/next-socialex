...
export const SeasonCard = ({ seasonNumber, episodesNumber, seasonTitle, episode1Title, route, imgSeason }: Props) => {
  return (
    <Link href={route} className="group relative flex flex-col rounded-md border border-white/10 bg-white/5 transition-all duration-300 hover:border-primary/50 hover:bg-white/10 hover:shadow-lg hover:shadow-primary/10" draggable={false}>
      <div className="w-full aspect-video relative overflow-hidden">
        <Image src={imgSeason} alt={episode1Title} width={320} height={240} className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-300" draggable={false} />
      </div>
