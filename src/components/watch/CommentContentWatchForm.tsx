...
export const CommentForm = ({ context }: Props) => {
  const [isPending, startTransition] = useTransition();
  const ref = useRef<HTMLTextAreaElement>(null);

  const handleSubmit = () => {
    const content = ref.current?.value ?? "";
    if (content.trim().length === 0) return toast.error("Agrega un comentario, no seas idiota");
    startTransition(async () => {
      const res = await createCommentWatchContext(context, content);
