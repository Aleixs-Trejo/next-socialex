...
export const PostReactionsUsers = ({
  allReactions,
  usersReactions,
  groupedReactions,
}: Props) => {
  ...
  const usersMap = limitNamesCount(usersReactions).map((user, idx) => (
    <li key={idx} className="text-sm text-gray-800 leading-4">
      {user}
    </li>
  ));
  ...
  return (
    <div className="relative group">
      ...
      >
        ...
        <span className="text-xs text-gray-200 select-none pointer-events-none">
          {allReactions}
        </span>
      </button>
      {usersReactions && usersReactions.length > 0 && (
        <div className="absolute z-5 opacity-0 transition-opacity pointer-events-none bottom-full left-0 w-max h-max bg-gray-200 rounded-md p-2 group-hover:opacity-100">
          <ul className="flex flex-col gap-1">{usersMap}</ul>
        </div>
      )}
      {showReactions && (
        <ModalReactions
