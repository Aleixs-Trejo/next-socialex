import { getAllLikesAndDislikesBySerie, getAllSeries } from "@/actions";
...
export const AllContentWatch = async () => {
  ...
  if (!resSeries.ok || !resSeries.data) return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"><span className="text-gray-400">Error al obtener series</span></div>;

  const series = resSeries.data;

  const serieIds = series.map(s => s.id);
  const resReactions = await getAllLikesAndDislikesBySerie(serieIds);
