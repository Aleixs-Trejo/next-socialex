...
export const PasswordStep = ({ form }: Props) => {
  const [showPassword, setShowPassword] = useState(false);

  const handleSubmit = (value: string) => {
    if (!value) return 'La contraseña es obligatoria';

    const result = onboardingSchema.shape.password.safeParse(value);
  ...
  return (
    <form.Field name="password" validators={{ onSubmit: ({ value }) => handleSubmit(value) }}>
      {field => (
